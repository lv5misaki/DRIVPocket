...
def index_points(points, idx):
    ...
    # new_points = torch.rand(1,idx.shape[1],points.shape[2])
    return new_points
...
def sample(npoint, xyz, feature, norm=None, sampling='fps'):
    B, N, C = xyz.shape
    xyz = xyz.contiguous()
    if sampling=='fps':
    ...
    return new_xyz, new_norm, new_feature
...
def RI_features(xyz, norm, new_xyz, new_norm, idx, group_all=False):
    ...
    if not group_all:
        grouped_xyz_local = grouped_xyz.clone() - new_xyz.view(B, S, 1, C)  # centered
    else:
        grouped_xyz_local = grouped_xyz.clone()  # treat orgin as center
    grouped_xyz_length = torch.norm(grouped_xyz_local, dim=-1, keepdim=True)
    #grouped_xyz_unit = grouped_xyz_local / (grouped_xyz_length+epsilon)
...
def sample_and_group(npoint, radius, nsample, xyz, norm, feature):
    """
    Input:
    ...
    new_xyz, new_norm, new_feature = sample(npoint, xyz, feature, norm=norm, sampling='fps')
    idx = group_index(nsample, radius, xyz, new_xyz, group='knn')
    #print(idx)
    ri_feat, idx_ordered = RI_features(xyz, norm, new_xyz, new_norm, idx)
    #print(new_xyz.shape)
    
    return new_xyz, ri_feat, new_norm, idx_ordered, new_feature
...
class RIConv2SetAbstraction(nn.Module):
    ...
    def forward(self, xyz, norm, points, feature, prev_mlp_convs=None, prev_mlp_bns=None):
        """
        Input:
            xyz: input points position data, [B, N, 3]
        ...
        if self.group_all:
            ...
            new_xyz, ri_feat, new_norm, idx_ordered, new_feature = sample_and_group(self.npoint, self.radius, self.nsample, xyz, norm, feature)
        
        feature = index_points(feature, idx_ordered)
        feature = feature.permute(0, 3, 2, 1)
        #print(feature.shape)
        # lift]
        ri_feat = ri_feat.permute(0, 3, 2, 1).detach()
        #print(ri_feat.shape)        
        ri_feat = torch.cat([ri_feat,feature], dim = 1)
        #ri_feat.detach()
        #idx_ordered.detach()
        #new_xyz.detach()
        #new_norm.detach()
        if prev_mlp_bns is None:
            for i, conv in enumerate(self.prev_mlp_convs):
                bn = self.prev_mlp_bns[i]
                ri_feat =  F.relu(bn(conv(ri_feat)))
        else:
            for i, conv in enumerate(prev_mlp_convs):
                bn = prev_mlp_bns[i]
                ri_feat =  F.relu(bn(conv(ri_feat)))
        ...
        return new_xyz, new_norm, ri_feat, new_feature
