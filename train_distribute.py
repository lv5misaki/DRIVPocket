...
from multi_train.distributed_utils import init_distributed_mode, dist, cleanup
...
def parse_args(argv=None):
    ...
    parser.add_argument('-d', '--data_dir', type=str, required=False,
                        help="Root directory of data", default="/home/dbw/ScPDB/scPDB")
    parser.add_argument('--train_recmolcache', type=str, required=False,
                        help="path to train receptor molcache", default="/home/dbw/PythonProject/DeepPocket_pp/scPDB_new.molcache2")
    parser.add_argument('--test_recmolcache', type=str, required=False,
                        help="path to test receptor molcache", default="/home/dbw/PythonProject/DeepPocket_pp/scPDB_new.molcache2")
    ...
    parser.add_argument('--step_when', type=int,
                        ...
                        default=2)
    parser.add_argument('--base_lr', type=float, help='Initial learning rate, default 0.001', default=0.000025)
    ...
    parser.add_argument('--DATA_ROOT', type=str, default='/home/dbw/PythonProject/DeepPocket_pp/coach420', help="the path of data root")
...
def train(gmaker_img, gmaker_mask, args, device, total=None):
    ...
    train_ProteinDataset = Protein_Dataset.TrainScPDB(args.train_types, data_root=args.data_dir,
                                              recmolcache=args.train_recmolcache, cache_structs=True,shuffle=True,stratify_receptor=False, balanced=False,)
    ...
    train_sampler = torch.utils.data.distributed.DistributedSampler(train_ProteinDataset)
    val_sampler = torch.utils.data.distributed.DistributedSampler(test_ProteinDataset)
    ...
    if args.usewandb :
        wandb.init(project='deep-pocket-pp', name=args.run_name + ' cuda:' + str(rank))
        wandb.watch(model)
    ...
    if 'SGD' in args.solver:
        ...
        optimizer = torch.optim.AdamW(model.parameters(), lr=args.base_lr, betas=(0.95, 0.99), weight_decay=args.weight_decay)
    elif 'Sophia' in args.solver:
        optimizer = SophiaG(model.parameters(), lr=args.base_lr, betas=(0.96, 0.999), rho=0.04, weight_decay=0.1)
    else:
    ...
    # scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', factor=args.step_reduce,patience=args.step_when, verbose=True)
    ...
    #scheduler = lr_scheduler.CosineAnnealingLR(optimizer, iterations, eta_min=5e-7 * args.world_size, last_epoch=-1)
    # scheduler = WarmupCosineAnnealingLR(optimizer, warmup_epochs=3, max_epochs=num_epochs+10, warmup_start_lr=1e-4, min_lr=1e-6)
    #scheduler = torch.optim.lr_scheduler.StepLR(optimizer,step_size=iterations,gamma=0.8)
    # scheduler = CyclicLR(optimizer,
    #                      base_lr=5e-7* args.world_size,
    #                      # Initial learning rate which is the lower boundary in the cycle for each parameter group
    #                      max_lr=5e-5* args.world_size,  # Upper learning rate boundaries in the cycle for each parameter group
    #                      step_size_up=4,  # Number of training iterations in the increasing half of a cycle
    #                      mode="triangular2"
    #                      cycle_mo)
    #scheduler = lr_scheduler.CosineAnnealingWarmRestarts(optimizer, iterations, T_mult=3, eta_min=5e-6 * args.world_size, last_epoch=-1)
    if args.checkpoint:
    ...
    #criterion = nn.BCELoss()
    dims = (28, box_size, box_size, box_size)
    ...
    for epoch in range(initial, num_epochs):
        ...
        dist.barrier()
        train_ProteinDataset = Protein_Dataset.TrainScPDB("/home/dbw/PythonProject/DeepPocket_pp/dataset_temp/epoch{}.txt".format(epoch),
        ...
        with tqdm(train_loader, desc="epoch:{}".format(epoch)) as train_bar:
            for batch in train_bar:
                ...
                mask_tensor, input_tensor,edge_pocket = mask_tensor.float().to(device),input_tensor.to(device), edge_pocket.to(device)
                optimizer.zero_grad()
                ...
                #point_pred = torch.where(point_pred > 0.5, 1, 0)
                # imbalance
                # print(acc)
                # running_acc += acc
                #edge_pred = (point_pred >0.5).float()

                # edge = get_edge(protein_coords, edge_pred, labels)
                # edge = edge.to(device)
                loss_point = Bceloss(point_pred, edge_pocket)
                # edge_fuse = 0.05 * edge + 0.1 * edge*masks_pred + 1*masks_pred
                ...
                # edge_fuse = (edge_fuse > 0.6).float()
                # fuse_dice = cal_dice_coeff(edge_fuse, mask_tensor)
                IOU = cal_IOU(pred, mask_tensor)
                # fuse_IOU = cal_IOU(edge_fuse, mask_tensor)
                loss_bce = criterion(masks_pred, mask_tensor)
                # Loss = 0.5*loss_dice + 2 * loss_bce + 1 * loss_point
                Loss = 2 * loss_bce + 1 * loss_point + 0.5 * loss_dice
                ...
                # print(IOU)
                if args.usewandb :
                    wandb.log({'train_loss': Loss.item(), 'loss_point':loss_point, 'loss_bce':loss_bce, 'train_accuracy': acc, 'train_dice': dice, 'train_IOU': IOU,
                               'learning rate': optimizer.param_groups[0]['lr']})
            # if epoch > 100 and iter % 100 == 0:
            #     coach_acc = test_Cemera3.get_acc(seg_model=model, DATA_ROOT=args.DATA_ROOT, args=args)
            #     if coach_acc > 0.91:
            #         torch.save({'model_state_dict': model.state_dict()},
            #                    '{}-{:.5f}-{}-{}.pth.tar'.format(outprefix, coach_acc, rank, epoch + 1))
            #     if coach_acc > best_coach_acc and rank == 0:
        ...
        #         print('the best coach acc = {}, epo={}'.format(best_coach_acc, str(epoch + 1)))
        #     if args.usewandb :
        #         wandb.log({'coach_acc': coach_acc})
        dist.barrier() 
        test_loss, test_acc, dice_coeff, IOU, total_fuse_dice,tatal_fuse_IOU,point_loss= test(model, test_loader, gmaker_img, gmaker_mask, args, criterion,
                                                    device)
        scheduler.step()
        #scheduler.step(dice_coeff)
        if test_loss < Bests['test_loss'] and rank == 0:
        ...
        if dice_coeff > Bests['dice_coeff'] and rank == 0:
...
def test(model, test_loader, gmaker_img, gmaker_mask, args, criterion, device):
    ...
    for batch in test_loader:
        ...
        # edge_fuse = 0.1 * edge + 0.4 * edge * pred + 1 * pred
        pred = (pred > 0.5).float()
        running_acc += torch.mean((mask_tensor == predictions).float()).detach().cpu()
        # edge_fuse = (edge_fuse > 0.5).float()
        # fuse_dice = cal_dice_coeff(edge_fuse, mask_tensor)
        # total_fuse_dice +=fuse_dice
    ...
    # total_fuse_dice = total_fuse_dice / (round(len(test_loader) / args.batch_size))
    dice_coeff = tot_dice / len(test_loader)
    # fuse_IOU = fuse_IOU / (round(len(test_loader) / args.batch_size))
