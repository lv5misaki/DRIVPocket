...
class TrainScPDB(MolDataset):
    def __init__(self, *args, **kwargs):
        ...
        self.gmaker_mask = molgrid.GridMaker(dimension=36, binary=True, gaussian_radius_multiple=-1, resolution=0.5)
        self.gmaker_img = molgrid.GridMaker(dimension=36, radius_scale=1.0)
    ...
    def __getitem__(self, item):
        ...
        riconv_tensor = pc_normalize(protein_coords,torch.tensor([float(labels[1]), float(labels[2]), float(labels[3])]), 18.25)
    ...
    def filter_points(self,Points, xyz, atomtypes, radii, distance_threshold= 32):
