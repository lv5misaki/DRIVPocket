...
class Grid2CoordsGradientFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_coords, grad_types):
        ...
        gmaker.backward_gradients(center, coords, types, radii, grid_gradient, grad_coords, grad_types, ddG, ddcoords,
                                  ddtypes)
...
class MolDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.Transform:
            centers = labels[1:]
            center_ = molgrid.float3(float(centers[0]), float(centers[1]), float(centers[2]))
            transformer = molgrid.Transform(center_, 1, True)
            transformer.forward(ex, ex)
        ...
        labels = [ex.labels[lab] for lab in range(self.num_labels)]
        return center, coords, atomtypes, radii, labels, ex.coord_sets
    ...
    def __getstate__(self):
        ...
        if self.typers is not None:  ## This will fail if self.typers is not none, need a way to pickle AtomTypers
            raise NotImplementedError(
                'MolDataset does not support pickling when not using the default Gnina atom typers, this uses %s'.format(
                    str(self.typers)))
    ...
    def collateMolDataset(batch):
        ...
        labels = []
        coord_sets = []
        for center, coords, types, radii, label, coord_set in batch:
        ...
        return lengths, centers, lcoords, ltypes, lradii, labels, coord_sets
